...
| **Cold tier (x86-Linux)** | **in progress (2026-06-17)** — step 1 done | **Step 1 DONE:** linux vstate snapshot wiring — `SaveState`/`RestoreState` events + `VcpuState`/`VmState` serialize/deserialize (the KVM `save_state`/`restore_state` already existed); compiles on amd64. **Remaining (mapped + de-risked):** (2) port the 4 device-manager persist methods `hvf/mmio.rs`→`kvm/mmio.rs` (mechanical — iterate `mmio_transports` + shared device-crate persist) + add `set_start_paused` to the linux `Vcpu`; (3) widen the ~15 `Vmm` cold-tier `cfg` gates `all(macos,aarch64)`→`any(…, all(linux,x86_64))` (bodies are platform-neutral: channel events + `vm.save_state()` + shared device mgr); (4) widen `builder.rs` restore path + `krun_start_enter` for linux; (5) `TestKrucibleSnapshotSuite` green on asus | a Linux/KVM x86 box (have: asus-i5) |
